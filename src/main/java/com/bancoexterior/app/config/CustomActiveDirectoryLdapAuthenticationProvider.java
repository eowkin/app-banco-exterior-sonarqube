...
public final class CustomActiveDirectoryLdapAuthenticationProvider extends AbstractLdapAuthenticationProvider{
	...
	private List<GrantedAuthority> createGrantedAuthoritiesFromLdapGroups(String[] groups) {
		...
		return addRole(groupNames);
		
	}

	public List<GrantedAuthority> addRole(List<String> groupNames) {
		
		List<GrantedAuthority> privilege = new ArrayList<>();
		for (String nombre : groupNames) {
			GrantedAuthority auxiliar = new SimpleGrantedAuthority(nombre);
			privilege.add(auxiliar);
		}
